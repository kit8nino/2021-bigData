...
df73 = dfper.where(dfper['Cocoa Percent']>73).groupby('Company Location').size()
notdf22 = dfper.where(dfper['Rating']<=3.1).groupby('Company Location').size()
df22 = dfper.where(dfper['Rating']>3.1).groupby('Company Location').size()
df3 = df2 / df1
df33 = df22/ df73
notdf33= notdf22 / df73 
 

EU_AF = ['Amsterdam', 'Austria', 'Belgium', 'Denmark', 'Finland', 'France', 'Germany', 'Ghana', 'Hungary', 'Iceland', 'Ireland', 'Italy', 'Lithuania', 'Poland', 'Portugal', 'Scotland', 'Spain', 'Sweden', 'Switzerland', 'U.K.', 'Wales']
